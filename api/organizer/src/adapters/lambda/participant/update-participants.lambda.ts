...
export const updateParticipants = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  let request: Request
  try {
    ...
    z.object({
      ...
      items: z.array(
        z.object({
          ...
          price: z.object({ amount: z.number(), currency: z.enum(allCurrencies) }),
        })
      ),
    })
  ),
