...
export const removeParticipants = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  let request: Request
  try {
...
const RequestSchema = z.object({
  ...
  participants: z.array(z.string().min(1, 'Participant id is required')),
})
