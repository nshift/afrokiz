...
export const approveGuests = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  let request: Request
  try {
...
const RequestSchema = z.object({
  ...
  guests: z.array(z.string().min(1, 'Guest id is required')),
})
