...
export const removeTickets = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  let request: Request
  try {
  ...
  try {
    ...
    await merchAdministration.removeTickets(request.eventId, request.tickets)
...
const RequestSchema = z.object({
  ...
  tickets: z.array(z.string().min(1, 'Pass id is required')),
})
