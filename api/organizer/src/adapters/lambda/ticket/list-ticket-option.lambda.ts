...
export const listTicketOption = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  let request: Request
  try {
  ...
  try {
    ...
    let ticketOptions = await merchAdministration.listTicketOption(request.eventId)
    return successResponse({ ticketOptions })
  } catch (error) {
    console.error(error)
    return internalServerErrorResponse(error)
  }
