...
export const listTicket = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  let request: Request
  try {
  ...
  try {
    ...
    let tickets = await merchAdministration.listTicket(request.eventId)
    return successResponse({ ticketOptions: tickets })
  } catch (error) {
    console.error(error)
    return internalServerErrorResponse(error)
  }
