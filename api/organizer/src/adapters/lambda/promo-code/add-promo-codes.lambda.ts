...
export const addPromoCodes = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  let request: Request
  try {
  ...
  try {
    ...
    await merchAdministration.addPromoCodes(request.eventId, request.promoCodes)
